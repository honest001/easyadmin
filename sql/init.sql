-- 新建数据库
CREATE DATABASE IF NOT EXISTS easyadmin DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE easyadmin;
...
CREATE TABLE sys_user (
  ...
  real_name varchar(50) character set utf8mb4 comment '真实姓名',
...
) comment='系统用户表' character set utf8mb4;
...
CREATE TABLE sys_role (
  ...
  permission varchar(500) comment '权限标识',
  status tinyint default 1 comment '1正常 0禁用',
  remark varchar(255) character set utf8mb4 comment '备注',
  create_time datetime default current_timestamp comment '创建时间',
  update_time datetime default current_timestamp on update current_timestamp comment '更新时间'
) comment='系统角色表' character set utf8mb4;

-- 初始化角色数据
INSERT INTO sys_role (role_name, role_code, permission, status) VALUES 
('超级管理员', 'admin', '*', 1),
('普通用户', 'user', 'user:*', 1);

-- 初始化管理员账号（密码：123456，明文）
...
-- 初始化商品分类数据
INSERT INTO goods_category (category_name, parent_id, sort, status) VALUES 
('电子产品', 0, 1, 1),
('服装', 0, 2, 1),
('食品', 0, 3, 1);
...
-- 初始化商品数据
INSERT INTO goods (goods_name, category_id, goods_spec, price, cost_price, stock, warn_stock, status) VALUES 
